...
module.exports = async function handler(req, res){
  ...
  try{
    ...
    const coachR = await sb(`/rest/v1/profiles?id=eq.${coachId}&select=stripe_price_id,stripe_price_id_annual,program_name`, token, {method:'GET'});
    ...
    let priceId = coach.stripe_price_id;
    if(plan === 'annual'){
      if(!coach.stripe_price_id_annual) return res.status(400).json({error:'Annual plan is not available yet — try monthly instead'});
      priceId = coach.stripe_price_id_annual;
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { coach_id: coachId, athlete_id: user.id, plan: plan||'monthly' }
    });
