...
function updatePaceVolLabels(s) {
  ...
  var elVolWrap = document.getElementById('e-s'+s+'-vol-wrap');
  ...
  if (elPaceWrap) elPaceWrap.style.display = 'block';
  if (elVolWrap) elVolWrap.style.display = 'block';
...
function updateVolPaceVisibility(s) {
  ...
  if (volWrap) volWrap.style.display = 'block';
  if (paceWrap) paceWrap.style.display = 'block';
}
...
function autoCalcPace(s) {
  var f = getModalityFlags(s);
  if (f.isEchoBike || f.isElliptical) return;
  ...
  if (f.isStair) {
    ...
    if (pm5 < 1) { pm5 = 1; ps5 = 0; }
    if (pm5 > 2 || (pm5 === 2 && ps5 > 59)) { pm5 = 2; ps5 = 59; }
...
function setPace(s, val) {
  var f = getModalityFlags(s);
  var unit = f.isEchoBike||f.isElliptical ? 'w' : f.isBike ? '/km' : f.isErg ? '/500m' : f.isStair ? 'steps/min' : 'min/km';
  var u = ' <span class="metric-unit">'+unit+'</span>';
  var us = ' <span class="metric-sm-unit">'+unit+'</span>';
  if (s === 1) {
...
function updateZoneBars(s) {
  ...
  [s===1 ? 'c1-zones-bars' : 'c2-s'+s+'-zones-bars', s===1 ? 'c2-s1-zones-bars' : null].forEach(function(bid) {
    if (!bid) return;
...
function updateTotZones() {
  ...
  if (hasData) {
    var normalized = {};
    for (var z=1;z<=5;z++) { normalized[z] = Math.round(combined[z]/total*100); }
  ...
  reader.onload = async function(e) {
    ...
    try {
      ...
      if (s===1) {
        ...
        if (parsed.zone1_pct||parsed.zone2_pct||parsed.zone3_pct||parsed.zone4_pct||parsed.zone5_pct) { setZonesFromImport(1,parsed); }
        if (type==='hyrox') {
        ...
        if (parsed.zone1_pct||parsed.zone2_pct||parsed.zone3_pct||parsed.zone4_pct||parsed.zone5_pct) { setZonesFromImport(2,parsed); }
        if (type==='hyrox') {
    ...
    } catch(err) { statusEl.textContent = 'Could not read — fill manually'; }
  };
...
async function saveSession() {
  ...
  var payload = {
    session_date: today, num_sessions: currentSess, training_phase: selectedPhase, division: selectedDivisions,
    s1_name: document.getElementById('e-s1-name').value,
    s1_workout_type: s1WorkoutType, s1_workout_subtypes: s1Subtypes,
...
function renderStats(data,period) {
  ...
  data.sessions.forEach(function(s){
    totalRun+=(s.tot_run_km||0); totalTime+=(s.tot_time_min||s.s1_duration_min||0);
  ...
  html+='</div><div class="stats-sessions-title">Sessions</div>';
