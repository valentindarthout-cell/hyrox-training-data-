...
function toggleRun(s, hidden) {
  if (s === 1) {
    ...
    var p1 = document.getElementById('c1-pace-block');
    var p2 = document.getElementById('c2-s1-pace-block');
    if (b1) b1.style.display = hidden ? 'none' : 'flex';
    if (b2) b2.style.display = hidden ? 'none' : 'flex';
    if (p1) p1.style.display = hidden ? 'none' : 'flex';
    if (p2) p2.style.display = hidden ? 'none' : 'flex';
  } else {
    var b = document.getElementById('c2-s2-run-block');
    var p = document.getElementById('c2-s2-pace-block');
    if (b) b.style.display = hidden ? 'none' : 'flex';
    if (p) p.style.display = hidden ? 'none' : 'flex';
  }
  updateTotRun();
}
...
function setTotCal(val) { sync('c-tot-cal', val + ' kcal'); }
    ...
    try {
      ...
      if (sessionNum === 1) {
        ...
        if (parsed.distance_km > 0) {
          document.getElementById('s1-norun').checked = false;
          toggleRun(1, false);
          document.getElementById('e-s1-run').value = parsed.distance_km;
          setRun(1, parsed.distance_km);
        } else if (parsed.distance_km === 0) {
          document.getElementById('s1-norun').checked = true;
          toggleRun(1, true);
        }
        ...
        if (parsed.distance_km > 0) {
          document.getElementById('s2-norun').checked = false;
          toggleRun(2, false);
          document.getElementById('e-s2-run').value = parsed.distance_km;
          setRun(2, parsed.distance_km);
        } else if (parsed.distance_km === 0) {
          document.getElementById('s2-norun').checked = true;
          toggleRun(2, true);
        }
